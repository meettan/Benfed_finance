...
/*Select Data from a table*/		
	...
	$sql = $this->db->query("SELECT 
	 1.1 'Version',
	 'GST' TaxSch,'B2B'  SupTyp,'N' RegRev,null EcmGstin,'N' IgstOnIntra,
	 'CRN' Typ,concat(a.invoice_no,'/1') No ,a.trans_dt Dt,
	 '19AABAT0010H2ZY' Gstin,'The West Bengal State Co-operative Marketing Federation Ltd.'LglNm,'BENFED'TrdNm,
	replace(replace(substr(d.addr,1,100),'\r\n',''),'/','-') Addr1,replace(replace(substr(d.addr,1,100),'\r\n',''),'/','-') Addr2,
	 d.district_name Loc,d.pin Pin,'19'Stcd,'9666666666'Ph,' info@benfed.org'Em,

	 c.gst_no Gstin1,c.cust_name LglNm1,c.cust_name TrdNm1,19 Pos,replace(replace(substr(c.cust_addr,1,100),'\r\n',''),'/','-')  Addr1_1,replace(replace(substr(c.cust_addr,1,100),'\r\n',''),'/','-')  Addr2_1,d.district_name Loc1,'45555666',d.pin Pin1,19 Stcd1,c.cnct_no Ph1,c.email_id Em1,

	 'The West Bengal State Co-operative Marketing Federation Ltd.' Nm2,replace(replace(substr(d.addr,1,100),'\r\n',''),'/','-') Addr1_2,'' Addr2_2,'W.B' Loc2,d.pin Pin2,19 Stcd2,
	 
	 c.gst_no Gstin2,c.cust_name LglNm2,c.cust_name TrdNm2,replace(replace(substr(c.cust_addr,1,100),'\r\n',''),'/','-')  Addr1_3,replace(replace(substr(c.cust_addr,1,100),'\r\n',''),'/','-')  Addr2_3,'W.B' Loc3,d.pin Pin3,19     Stcd3,
	 
	 '1' SlNo,b.product_desc PrdDesc,'N' IsServc,0 HsnCd,''Barcde,a.qty Qty,'0'FreeQty,'' Unit,1 UnitPrice,
	a.taxable_amt TotAmt,0 Discount, 0 PreTaxVal,a.taxable_amt AssAmt,b.cgst_rt*2 GstRt,0 IgstAmt,a.cgst_rt  CgstAmt,a.sgst_rt SgstAmt,
	 0 CesRt,0 CesAmt,0 CesNonAdvlAmt,0 StateCesRt,0 StateCesAmt, 0 StateCesNonAdvlAmt,0 OthChrg,a.taxable_amt+a.cgst_rt+a.sgst_rt TotItemVal,
	 0 OrdLineRef,'IN' OrgCntry,0 PrdSlNo,
	 '' Nm4,
     NULL   ExpDt,
     NULL  WrDt,
     ...
     0 OthChrg,
     0 RndOffAmt,
     a.taxable_amt + a.cgst_rt+ a.sgst_rt TotInvVal,
     0 TotInvValFc,
	 ''  Nm6,
	 0  AccDet,
	 '' Mode,
	 ...
	 '' ShipBNo,
	  ...
	  ''  VehType,
	  1  TransMode
	from td_rent_collection a  ,md_rent_product b,md_rent_customer c,md_district d
	where a.prod_id=b.sl_no
	
	and a.cust_id=c.id
	and c.cust_dist=d.district_code
    and d.district_code=342
		...
		/*$sql = $this->db->query("SELECT sp_mt
		 ...
		 $sql = $this->db->query("SELECT sp_mt,mrp_gst,sale_rtgst,sp_bag_gst
									from  mm_sale_rate		   							
										 ...
										 and district='$br_cd'
													and comp_id='$comp_id'
													and prod_id ='$prod_id' and catg_id = '$category')");
			// return $sql->row();
			return $sql->result();
		}
...
function f_salecrjnl($data){
		...
		function f_salejnl($data){
			...
			curl_setopt_array($curl, array(
			CURLOPT_URL => FIN_BASE_URL.'index.php/api_voucher/sale_voucher',
			CURLOPT_RETURNTRANSFER => true,
			CURLOPT_ENCODING => '',
			  CURLOPT_MAXREDIRS => 10,
			  CURLOPT_TIMEOUT => 0,
